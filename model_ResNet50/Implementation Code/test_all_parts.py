...
class CustomModel(pl.LightningModule):
    ...
    def get_predictions_and_labels(self, phase):
        ...
        with torch.no_grad():
            for inputs, labels in self.dataloaders[phase]:
                ...
                predicted_probabilities = torch.softmax(outputs, dim=1)[:, 1]

                all_predictions.extend(predicted.cpu().numpy())
                all_labels.extend(labels.cpu().numpy())
                all_predicted_probabilities.extend(predicted_probabilities.cpu().numpy())

        if phase == "train":
            numpy.save("train_all_labels.npy", all_labels)
            numpy.save("train_all_preds.npy",all_predictions)
            numpy.save("train_all_probabilities", all_predicted_probabilities)
        if phase == "val":
            numpy.save("val_all_labels.npy", all_labels)
            numpy.save("val_all_preds.npy", all_predictions)
            numpy.save("val_all_probabilities", all_predicted_probabilities)
    ...
    def on_validation_epoch_end(self):
        ...
        confusion_df = pd.DataFrame(cm, index=class_labels, columns=class_labels)
        print(confusion_df)
        numpy.save("confusion_df", confusion_df)
        # Extract individual values from the confusion matrix
        # print(cm)
...
"""
   Create transformations for data augmentation and normalization.

       probability (float): Probability for applying random grayscale transformation.
       brightness_range (list): Range for adjusting brightness.
       contrast (float): Value for adjusting contrast.
       saturation (float): Value for adjusting saturation.
       hue (float): Value for adjusting hue.

       train_transform (torchvision.transforms.Compose): Composition of transformations for training data.
       val_transform (torchvision.transforms.Compose): Composition of transformations for validation data.
       test_transform (torchvision.transforms.Compose): Composition of transformations for test data.
"""
...
"""
    Create custom datasets and corresponding data loaders.
    ...
    Returns:
        dataloaders (dict): Dictionary containing data loaders for train, validation, and test datasets.
"""

train_dataset = MBH_dataset(data_dir='/home/woody/iwfa/iwfa011h/Fangjun_Wang_2021_MBH-dataset/MBH-dataset/',
                                    label_file="/home/woody/iwfa/iwfa011h/Fangjun_Wang_2021_MBH-dataset/MBH-dataset/Labels/All_Parts/model_train.txt",
                                    transform=train_transform)
train_dataset.load_data()

val_dataset = MBH_dataset(data_dir='/home/woody/iwfa/iwfa011h/Fangjun_Wang_2021_MBH-dataset/MBH-dataset/',
                          label_file="/home/woody/iwfa/iwfa011h/Fangjun_Wang_2021_MBH-dataset/MBH-dataset/Labels/All_Parts/model_val.txt",
                          transform=val_transform)
val_dataset.load_data()

test_dataset = MBH_dataset(data_dir='/home/woody/iwfa/iwfa011h/Fangjun_Wang_2021_MBH-dataset/MBH-dataset/',
                          label_file="/home/woody/iwfa/iwfa011h/Fangjun_Wang_2021_MBH-dataset/MBH-dataset/Labels/All_Parts/model_test.txt",
...
params = {
    'batch_size': 8,
    'lr': 0.0046259912779318966,
    'optimizer_name': 'SGD',
    'dropout_rate': 0.4155036664574021,
    'unfreeze_layers': 'from_layer4'
}
...
writer = SummaryWriter(log_dir="tensorboard_ALL_PARTS/")
...
csv_logger = CSVLogger(save_dir="/home/hpc/iwfa/iwfa011h/Code/Code/Code/Code/Code/Code/ResNet50/logs/train", name="metrics_pr")

trainer = pl.Trainer(
    accelerator="auto",  # Use GPU if available
    max_epochs=100,
    callbacks=[pl.callbacks.EarlyStopping(monitor='val_loss', patience=15, mode='min', verbose = True)], logger=csv_logger
)
...
# Evaluate the model on the validation dataset to get predicted probabilities
model.eval()
all_labels = []
all_predicted_probabilities = []
...
with torch.no_grad():  # Disable gradient computation
    for inputs, labels in dataloaders['val']:
        ...
        predicted_probabilities = torch.softmax(outputs, dim=1)[:, 1]  # Probabilities for class 1 (positive class)
...
torch.save(model.state_dict(), 'resnet_part_all_pr_curve.pth')
...
def test_model(model, dataloader):
    ...
    file_path = "resnet_part_all_test.csv"
